...
e&&(s=a,a=a[0]));if(!a||1!==a.nodeType)a=document.createElement(c||"div");if(l&&g){for(l>m&&(g=q);l<m--;)g=g.parentNode;g.appendChild(a)}else j.push(g=a);m=l;q=a;if(d){d=d.replace(u,function(b,c,d){a.setAttribute(c,d||c);return""});b=d.indexOf(" ");-1!==b&&(c=d.substr(b+1).replace(w,y),d=d.substr(0,b),0>e.inArray(a.tagName.toLowerCase(),x)?a.innerHTML=c:a.value=c);for(;null!==(b=v.exec(d));)switch(c=b[2],b[1]){case ".":k.push(c);break;case "#":a.id=c;break;case "$":r[c]=s||e(a)}k.length&&(k.push(a.className),
